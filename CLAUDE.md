...
- **Content-Security-Policy header.** Not set today (Inertia + Vue work without it). Adding a strict CSP (`default-src 'self'`; `script-src 'self'`; `style-src 'self' 'unsafe-inline'` for Tailwind utilities; `connect-src 'self'`) is defense-in-depth against XSS-as-a-second-stage. Tricky in dev with Vite HMR (ws://localhost:5173), implement gated by `app()->environment('production')` in a dedicated middleware.
